<script lang="ts">
	...
	const createNote = async () => {
		...
		creating = true;
		const title = newNoteTitle.trim() || $i18n.t('Simple Mode Note');
...
<Modal bind:show size="sm" className="bg-white dark:bg-gray-900 rounded-3xl">
	<div class="flex flex-col max-h-[80vh]">
		...
		<div class="p-5 border-b border-gray-100 dark:border-gray-800 space-y-3">
			<label class="block text-sm text-gray-600 dark:text-gray-300" for="simple-mode-note-title">
				{$i18n.t('Create Note')}
			</label>
			<div class="flex gap-2">
				<input
					id="simple-mode-note-title"
					...
					placeholder={$i18n.t('Simple Mode Note')}
					on:keydown={(event) => {
						if (event.key === 'Enter') {
							createNote();
						}
