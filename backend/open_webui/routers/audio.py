...
async def create_transcription_session_endpoint(
    request: Request,
...
async def upload_transcription_chunk(
    ...
    ext = safe_name.rsplit('.', 1)[-1].lower() if '.' in safe_name else 'webm'
    if allowed_extensions and ext.lower() not in [allowed_ext.lower() for allowed_ext in allowed_extensions]:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    try:
        result = await asyncio.to_thread(transcribe, request, transcription_path, metadata, user)
    except HTTPException:
        raise
    except Exception as e:
        log.exception(e)
...
async def cancel_transcription_session(
    request: Request,
    session_id: str,
    user=Depends(get_verified_user),
):
